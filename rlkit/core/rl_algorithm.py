...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
            ...
            num_exp_traj_eval=1,
            update_post_train=1,
            num_updates_global=1000,
            num_updates_task=1000,
            global_update_interval=200,
            before_train=0,
            max_traj=100,
            seed=1,
        ...
        self.temp_num_iterations = copy.deepcopy(self.num_iterations)
        ...
        self.video_start_episode = video_start_episode
        self.seed = seed
    ...
    def train(self):
        ...
        for env_id in range(len(self.env_set)*2):
            #env = self.env_set[env_id]
            self.update_step = 0
            #if self.backward_test:
            if self.backward_test and env_id>=len(self.agent.forw_dyna_set):
                backward=True
            else:
                backward=False
                #backward mode
            if backward:
                ...
                self.global_update_interval = copy.deepcopy(self.max_path_length)
            ...
            for it_ in gt.timed_for(
                ...
                if backward:
                    self._start_epoch(it_ + (env_id-len(self.agent.forw_dyna_set)) * self.num_iterations + len(self.agent.forw_dyna_set)*self.temp_num_iterations)
                    if it_ % 10 == 0:
                        prt = True
                    else:
                        prt = False
                ...
                if it_ == 0 and backward == False:
                    self.pretrain(env_id)
                    print('done for pretraining')
                    self.new_task = True
                else:
                    self.new_task = False

                if self.backward_test and it_ >= (self.num_iterations-3):
                    test=True
                else:
                    test=False

                self.try_to_train(env_id, backward, prt, iter=it_, singlemodel=False, test=test)
                ...
                # eval
                if (self.start_train and (self.task_step // self.global_update_interval != self.update_global)) or test:
                #if self.start_train or test:
                    if backward:
                        self._try_to_eval(it_ + (env_id-len(self.agent.forw_dyna_set)) * self.num_iterations + len(self.agent.forw_dyna_set)*self.temp_num_iterations)
                    else:
    ...
    def try_to_train(self, env_id, backward=False, prt=False,iter=0, singlemodel=False, test=False):
        if backward:
        ...
        if not backward:
            if iter<2:
                single=True
        #TODO change max samples
        paths, n_samples, info = self.sampler.obtain_samples(env_idx, max_samples=self.max_path_length,
                                                             max_trajs=self.max_traj, planning=self.start_train, backward=backward, prt=prt, singlemodel=single)
        ...
        if self.task_step >= self.before_train:
            ...
            if self.backward_test and test:
                self._do_training(env_idx, True, single_model=singlemodel, update_post=False, test=test)

            else:
                self._do_training(env_idx, backward, single_model=singlemodel, update_post=False)
            suc_num = 0
            if (self.task_step // self.global_update_interval != self.update_global) or test:
                all_rets = []
                online_returns = []

                suc_num += info['success']
                ...
                self.eval_statistics['success'] = suc_num
